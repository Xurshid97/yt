...
#             for fmt in formats:
#                 if fmt.get("vcodec") != "none":  # Ignore audio-only formats
#                     height = fmt.get("height")
#                     format_id = fmt.get("format_id")
#                     if height and format_id:
#                         available_formats[str(height)] = format_id  # Store height as key
...
#     """ Download YouTube video in the selected quality, or the best available. """
#     available_formats = get_available_formats(url)

#     # Check if requested quality is available, otherwise choose the best match
#     if quality in available_formats:
#         format_id = available_formats[quality]
#     else:
#         best_quality = max(available_formats.keys(), key=int)
#         format_id = available_formats[best_quality]  # Use best available quality
#         logging.warning(f"Requested quality {quality} not available. Using {best_quality}p instead.")
...
# # Dictionary to store user requests (user_id → YouTube URL)
# user_requests = {}
...
#         user_requests[user_id] = url  # Store URL for this user

#         # Fetch available formats
#         available_formats = get_available_formats(url)

#         if not available_formats:
#             await update.message.reply_text("No video formats available. Please try another link.")
#             return

#         # Generate buttons dynamically based on available formats
#         keyboard = [[InlineKeyboardButton(f"{quality}p", callback_data=quality)] for quality in sorted(available_formats.keys(), key=int)]
...
#     quality = query.data  # Extract chosen quality
#     user_id = query.from_user.id
#     url = user_requests.get(user_id)  # Retrieve URL

#     if not url:
#         await query.message.reply_text("Error: No URL found. Please send the YouTube link again.")
#         return

#     await query.edit_message_text(f"Downloading video in {quality} quality... Please wait.")

#     video_path = download_youtube_video(url, quality)
#     file_size = get_file_size(video_path)

#     if file_size <= 50:  # Send directly if ≤50MB
#         await context.bot.send_video(chat_id=user_id, video=open(video_path, "rb"))
#     else:  # Upload via uploader bot if >50MB
#         await query.message.reply_text(f"Video is {file_size:.2f}MB. Uploading via bot...")
...
#                     )
#                     await query.message.reply_text("Upload completed and sent to the requester.")
#                 else:
#                     await query.message.reply_text("Failed to retrieve uploaded video.")

#             except Exception as e:
#                 await query.message.reply_text(f"Error: {e}")

# # can we somehow hide the user before forward the video
